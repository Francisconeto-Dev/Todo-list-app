...
$('form').on('submit', function(e){
    ...
    if(newtarefa.trim() !==""){
        ...
        $(tarefaadd).appendTo('#task-list');
        $('#nova-tarefa').val(''); //limpando o formulario depois do loop
        
        $(tarefaadd).find('.task-checkbox').change(function(){
            $(this).siblings('span').toggleClass('completed', this.checked);
         
        });
    
        $(tarefaadd).find('.edit-icon').click(function(){

            const currentText = $(this).siblings("span").text();

            $('#nova-tarefa').val(currentText);

            $('#submitbtn').text('Salvar').off('click').on('click', function(e){
                e.preventDefault();
                const updatedText = $('#nova-tarefa').val().trim();
                
                if(updatedText !== ""){
                    $(tarefaadd).find('span').text(updatedText);
                    $('#nova-tarefa').val('');
            ...
            $(tarefaadd).find('.delete-icon').click(function(){
